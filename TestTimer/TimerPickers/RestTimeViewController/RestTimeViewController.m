...
int indexForSelectedRestTimeColorCheckMark=0;
...
- (void)updateInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation {
    ...
    [_restTimeCollectionView reloadData];
}
...
- (void)viewDidLoad {
    ...
    {
        _restTimeCollectionView.hidden = YES;
    }
    else{
        _restTimeCollectionView.hidden = NO;
    }
    [self dataFromArraysToCollectionView];
     [_restTimeCollectionView registerNib:[UINib nibWithNibName:@"SettingsManagerThemeCollectionViewCell" bundle:[NSBundle mainBundle]] forCellWithReuseIdentifier:@"SettingsManagerThemeCollectionViewCell"];
    self.title=@"Rest".localized;
    ...
    for ( int i=0; i<60; i++) {
        ...
        if (i<60) {
            [secondsArray addObject:stringValues];
        }
        [minutesArray addObject:stringValues];
    }
}
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [_restTimeCollectionView reloadData];
    if (_selectedRestTimeIndexInCollectionView==0) {
        _selectedRestTimeIndexInCollectionView=5;
    }
     if(_selectedRowinMin==0 && _selectedRowinSec==3){
        [_restTimePicker selectRow:0 inComponent:kMinutesComponent animated:YES];
        [_restTimePicker selectRow:3 inComponent:kSecondsComponent animated:YES];
    }
    else{
        [_restTimePicker selectRow:_selectedRowinMin inComponent:kMinutesComponent animated:YES];
        [_restTimePicker selectRow:_selectedRowinSec inComponent:kSecondsComponent animated:YES];
    }
...
- (IBAction)doneButtonTapped:(id)sender {
    _selectedRowinMin = [_restTimePicker selectedRowInComponent:kMinutesComponent];
    _selectedRowinSec = [_restTimePicker selectedRowInComponent:kSecondsComponent];
     _fullValueSeconds= (_selectedRowinMin*60)+(_selectedRowinSec%60);
    if (_selectedRowinMin==0 && _selectedRowinSec==0) {
        
        _selectedRestTimeIndexInCollectionView= 0;
    }
    [self delegateWorkTimeViewController];
    [self.navigationController popViewControllerAnimated:YES];
    [_restTimePicker reloadAllComponents];
}
                 ...
                 cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.item == _selectedRestTimeIndexInCollectionView)
    {
...
{
    indexForSelectedRestTimeColorCheckMark = (int) indexPath.item;
    _selectedRestTimeColor =settingsCircleThemeTitles[indexForSelectedRestTimeColorCheckMark];
    _selectedRestTimeIndexInCollectionView = indexForSelectedRestTimeColorCheckMark;
    [_restTimeCollectionView reloadData];
}


-(void) delegateWorkTimeViewController{
    if ([_delegate respondsToSelector:@selector(dataFromRestTimeMinViewController:pickedMinuteValue:pickedSecondValue:pickedColor:)]) {
        [_delegate dataFromRestTimeMinViewController:_fullValueSeconds pickedMinuteValue:_selectedRowinMin pickedSecondValue:_selectedRowinSec pickedColor:_selectedRestTimeIndexInCollectionView];
    }
}
